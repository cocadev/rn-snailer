export const findCityAndState = (postcode) => {
  let selected;
  states.forEach((state) => {
    if (state.prefix.includes(postcode[0])) {
      if (selected) return;
      state.city.forEach((c) => {
        if (selected) return;
        c.postcode.forEach((p) => {
          if (p === postcode) {
            selected = {city: c.name, state: state.name};
            return;
          }
        });
      });
